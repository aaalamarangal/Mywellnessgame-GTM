# Demo Posts Log
Parent: 3.Recognising fulfilling supply | CLAUDE.md

## Purpose

One entry per demo post published on LinkedIn.
This tracks what was posted, what response it got, and what it taught.

---

## Template

```
---
## [Post title / first line] — [Date]

**Post link:** [LinkedIn URL]
**Game:** Clarity / Action / Lesson

**Full text:**
> [exact post text]

**Engagement:**
- Impressions:
- Comments:
- Notable responses:

**What I learned:**
---
```

---

## Entries

---
## Post 3 — "The discipline was always there. The visibility wasn't." — March 8, 2026

**Post link:** https://www.linkedin.com/feed/update/urn:li:activity:7436424084780941312/
**Game:** Action (the founder knows what to do — they can't see which one is the one)

**Draft history:**
- Draft A — rejected before posting. Hook opened with "The Leverage Stack." Too pitchy.
- Draft B — posted on LinkedIn, then deleted. Five failure points: hook too abstract, framework-heavy body, too long, P.S. pitch, teaching tone.
- Draft C — rebuilt from diagnosis. All five failure points addressed.

**Full text:**
> Most founders I talk to don't have a discipline problem.
>
> They show up. They have tools. Some at their desk at 5am.
>
> The problem: they cannot see which move matters today. Not generally. Specifically.
>
> When the move is visible — the pull replaces the push.
>
> A founder told me: "It is building courage."
>
> He didn't mean motivation. He meant he stopped pushing and started moving.
>
> The discipline was always there. The visibility wasn't.

**Word count:** 72

**What Draft C fixed:**
- Hook: opens with a specific observation — "discipline problem" creates immediate self-recognition
- Body: zero framework terms — describes the experience, not the architecture
- Length: 72 words
- No P.S., no CTA
- Tone: observation from field, not instruction

**Engagement:**
- Impressions:
- Comments:
- Notable responses:

**What I learned:**
—
