...
11. **At the start of every session** — check the Post Arc Tracker in post-inspiration.md. If one entry is marked "In Progress" — name it immediately: "Your active post is [#X — title]. Did you make any progress since last session?" If no entry is "In Progress" — ask Shiva to pick one from the Captured list before doing anything else. At the end of every session — before closing, ask: "Did you make progress on [post title] today? Should I mark it Published or keep it In Progress?"
12. **Always maintain a Claude suggestion in the Post Arc Tracker** — after checking the active post (Rule 11), scan the tracker for any row tagged "Claude suggestion." If one exists and is Captured — name it: "My current suggestion is [#X — title]. Do you want to develop that next or pick from the Captured list?" If no Claude suggestion exists — read demo-message-log.md and propose one new post angle based on the meta-pattern you observe across all interactions. Add it as a new row in the tracker before responding. The goal is always two posts in motion: one In Progress, one Claude suggestion waiting.